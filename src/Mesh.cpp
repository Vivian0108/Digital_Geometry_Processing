...
Mesh::decimateQuadricEdgeCollapse()
{
  ...
  Edge * minEdge = &(*(edgesBegin()));
  // Finding min error edge
  for (EdgeIterator ei = edges.begin(); ei != edges.end(); ++ei) {
  	if (ei->getQuadricCollapseError() < 0.0)
  	  continue;
  	else if (ei->getQuadricCollapseError() < minError) {
  		minError = ei->getQuadricCollapseError();
  		minEdge = &(*ei);
  	}
  }

  Vector3 optPos = minEdge->getQuadricCollapsePosition();
  MeshVertex * newVertex;

  // Collapsing min error edge after checking validity
  if (minError == std::numeric_limits<double>::infinity())
  	return NULL;

  newVertex = collapseEdge(minEdge);

  // Changing vertex position from endpoint of the collapsed edge to optimum position
  newVertex->setPosition(optPos);

  // Updating face normals
  for (Vertex::FaceIterator fi = newVertex->facesBegin(); fi != newVertex->facesEnd(); ++fi)
  	(*fi)->updateNormal();

  // Updating quadric and normal of this vertex
  newVertex->updateQuadric();
  newVertex->updateNormal();

  /** For every edge incident on this vertex, update quadric & normal for other endpoint and update quadric collapse error 
  	  and optimal collapse position for the edge */
  for (Vertex::EdgeIterator ei = newVertex->edgesBegin(); ei != newVertex->edgesEnd(); ++ei) {
	((*ei)->getOtherEndpoint(newVertex))->updateQuadric();
	((*ei)->getOtherEndpoint(newVertex))->updateNormal();
...
void
Mesh::draw(Graphics::RenderSystem & render_system, bool draw_edges, bool draw_normals, bool use_vertex_data, bool send_colors) const
{
  ...
  if (draw_normals)
  {
    glPushMatrix();
      ...
      render_system.beginPrimitive(Graphics::RenderSystem::Primitive::LINES);
        ...
        render_system.sendVertex(vi->getPosition() + vi->getNormal());
      render_system.endPrimitive();
      }
    glPopMatrix();
  }
