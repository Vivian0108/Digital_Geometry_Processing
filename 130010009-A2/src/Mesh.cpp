...
Mesh::decimateQuadricEdgeCollapse()
{
  ...
  debugAssertM(numFaces()>0,"There are no Faces!");
  // TODO
  Edge * min_edge = &(*edgesBegin());
  double error;
  double min_error = std::numeric_limits<double>::infinity();
  
  for (EdgeIterator ei = edgesBegin(); ei != edgesEnd(); ++ei){
    error = ei->getQuadricCollapseError();
    if(error>=0 && error < min_error ){
      min_error = error;
     min_edge = &(*ei);
    }
  }
  if(min_error == std::numeric_limits<double>::infinity()){
    return NULL;
  }
  Vertex * u = min_edge->getEndpoint(0);
  //Vertex * v = min_edge->getEndpoint(1);
  Vector3 w = min_edge->getQuadricCollapsePosition();

  u = collapseEdge(min_edge);
  u->setPosition(w);

  

   for (Vertex::FaceIterator vfi = u->facesBegin(); vfi != u->facesEnd(); ++vfi) {
   ...
   for (Vertex::EdgeIterator vei = u->edgesBegin(); vei != u->edgesEnd(); ++vei){
        ...
        //Edge * u_edge = &(*vei);
        Vertex * oep = (*vei)->getOtherEndpoint(u);
        oep->updateQuadric();
        oep->updateNormal();
        (*vei)->updateQuadricCollapseError();
   }
