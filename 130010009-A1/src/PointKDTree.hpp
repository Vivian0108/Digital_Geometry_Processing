...
class PointKDTree : private Noncopyable
{
  ...
  public:
    ...
    void getting_kdtree(Node* root1,std::vector<Point *> const & points) ;   
    ...
    void rangeQuery(Node* node,RangeT const & range, std::vector<Point *> & points_in_range) const
    {
      // TODO
      
      if(range.intersects(node->bbox) == false){
        return;
      }
      if(node->points.size() <=5){                            //if the node is a leaf node
        for(size_t i =0;i< node->points.size();i++){
          if(range.contains(node->points[i]->getPosition())) {
            points_in_range[i] = node->points[i];
          }
        }
      }
      if(node->lo && range.intersects(node->lo->bbox)){
        rangeQuery(node->lo,range,points_in_range);
      }
      if(node->hi && range.intersects(node->hi->bbox)){
        rangeQuery(node->hi,range,points_in_range);
      }
    ...
    {
      rangeQuery(root,range,points_in_range);//calling the helper function
    }
